...
class TestAccountListMessengersForUser:
    ...
    def test_skype_account(self, dc_api: DcApiWithToken):
        ...
        with allure.step("Идентификатор аккаунта"):
            check.equal(parsed_data["data"][0]["accountID"], 10)
        with allure.step("Логин"):
            check.equal(
                parsed_data["data"][0]["accountName"], "Viber_REST_test_user_cyrillic"
            )
        ...
        with allure.step("Имя аккаунта"):
            check.equal(
                parsed_data["data"][0]["displayName"], "Viber_REST_test_user_cyrillic"
            )
    ...
    def test_icq_account(self, dc_api: DcApiWithToken):
        ...
        parsed_data = json.loads(data_str)
        with allure.step("Идентификатор аккаунта"):
            check.equal(parsed_data["data"][1]["accountID"], 9)
        with allure.step("Логин"):
            check.equal(
    ...
    def test_icq_account(self, dc_api: DcApiWithToken):
        ...
        parsed_data = json.loads(data_str)
        with allure.step("Идентификатор аккаунта"):
            check.equal(parsed_data["data"][2]["accountID"], 8)
        with allure.step("Логин"):
            check.equal(
                parsed_data["data"][2]["accountName"],
