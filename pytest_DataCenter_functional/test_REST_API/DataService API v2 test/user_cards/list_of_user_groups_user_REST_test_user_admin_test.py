...
class TestListOfUserGroups:
    ...
    def test_control_group_group(self, dc_api: DcApiWithToken):
        ...
        with allure.step(
            ...
            check.equal(data_dict["data"][3]["branch"], 4)
        with allure.step("Цвет профиля"):
            check.equal(data_dict["data"][3]["color"], 1)
        with allure.step("Имя группы"):
            check.equal(data_dict["data"][3]["displayName"], "control group")
        with allure.step("Идентификатор группы"):
            check.equal(data_dict["data"][3]["groupID"], 92)
        with allure.step("Флаг, находится ли пользователь в группе"):
            check.is_true(data_dict["data"][3]["inGroup"])
        with allure.step("Родитель группы"):
            check.equal(data_dict["data"][3]["parentID"], 4)
        with allure.step("Тип группы"):
            check.equal(data_dict["data"][3]["type"], 102)
