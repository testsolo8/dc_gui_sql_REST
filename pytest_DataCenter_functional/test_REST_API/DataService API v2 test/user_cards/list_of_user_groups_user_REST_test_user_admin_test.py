...
url_tail = "/api/v2/data_center/user_cards/groups_user?userID=5&inGroup=1"


@allure.epic("DataService API v2 (user_cards)")
@allure.feature("List of user groups REST_test_user_admin")
@pytest.mark.testRESTAPI
@allure.story("Список групп пользователя REST_test_user_admin")
class TestListOfUserGroups:
    ...
    @allure.title("Группа control group")
    def test_control_group_group(self, dc_api: DcApiWithToken):
        data_dict = dc_api.req_get(url_tail).json()
        data_str = json.dumps(data_dict)
        parsed_data = json.loads(data_str)
        with allure.step(
            ...
            check.equal(parsed_data["data"][3]["branch"], 4)
        with allure.step("Цвет профиля"):
            check.equal(parsed_data["data"][3]["color"], 1)
        with allure.step("Имя группы"):
            check.equal(parsed_data["data"][3]["displayName"], "control group")
        with allure.step("Идентификатор группы"):
            check.equal(parsed_data["data"][3]["groupID"], 92)
        with allure.step("Флаг, находится ли пользователь в группе"):
            check.is_true(parsed_data["data"][3]["inGroup"])
        with allure.step("Родитель группы"):
            check.equal(parsed_data["data"][3]["parentID"], 4)
        with allure.step("Тип группы"):
            check.equal(parsed_data["data"][3]["type"], 102)
