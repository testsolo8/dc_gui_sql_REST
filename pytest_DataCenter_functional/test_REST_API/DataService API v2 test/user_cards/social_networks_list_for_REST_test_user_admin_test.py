...
class TestSocialNetworksListForUser:
    ...
    def test_skype_account(self, dc_api: DcApiWithToken):
        ...
        with allure.step("Идентификатор аккаунта"):
            check.equal(parsed_data["data"][0]["accountID"], 6)
        with allure.step("Логин"):
            check.equal(
                parsed_data["data"][0]["accountName"], "Vkontakte_REST_test_user_admin"
            )
        ...
        with allure.step("Имя аккаунта"):
            check.equal(
                parsed_data["data"][0]["displayName"], "Vkontakte_REST_test_user_admin"
            )
    ...
    def test_icq_account(self, dc_api: DcApiWithToken):
        ...
        parsed_data = json.loads(data_str)
        with allure.step("Идентификатор аккаунта"):
            check.equal(parsed_data["data"][1]["accountID"], 5)
        with allure.step("Логин"):
            check.equal(
        ...
        with allure.step("Тип аккаунта - UC_AccountsType"):
            check.equal(parsed_data["data"][1]["accountTypeID"], 2)
        with allure.step("Тип контакта: 3 Мессенджеры, 4 Соц.Сети, 5 Другие"):
    ...
    def test_icq_account(self, dc_api: DcApiWithToken):
        ...
        parsed_data = json.loads(data_str)
        with allure.step("Идентификатор аккаунта"):
            check.equal(parsed_data["data"][2]["accountID"], 4)
        with allure.step("Логин"):
            check.equal(
                parsed_data["data"][2]["accountName"], "Instagram_REST_test_user_admin"
            )
        with allure.step("Тип аккаунта - UC_AccountsType"):
            check.equal(parsed_data["data"][2]["accountTypeID"], 4)
        with allure.step("Тип контакта: 3 Мессенджеры, 4 Соц.Сети, 5 Другие"):
            check.equal(parsed_data["data"][2]["contactTypeID"], 4)
        with allure.step("Имя аккаунта"):
            check.equal(
                parsed_data["data"][2]["displayName"], "Instagram_REST_test_user_admin"
            )
        with allure.step("Идентификатор документа в индексе"):
