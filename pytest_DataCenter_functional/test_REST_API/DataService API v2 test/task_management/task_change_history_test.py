...
class TestTaskChangeHistory:
    ...
    def test_change_history_task_2(self, dc_api: DcApiWithToken):
        ...
        with check.check, allure.step("Дата изменения"):
            check.equal(data_dict["data"][0]["changeDateTime"], 1637740630)
        with check.check, allure.step("Дата изменения статуса"):
            check.is_none(data_dict["data"][0]["changeStateDateTime"])
        ...
        with check.check, allure.step("Дата создания"):
            check.equal(data_dict["data"][0]["createDateTime"], 1637740630)
        ...
        with check.check, allure.step("deleted"):
            check.is_false(data_dict["data"][0]["deleted"])
        with check.check, allure.step("Описание задачи"):
            check.equal(data_dict["data"][0]["description"], "")
        with check.check, allure.step(
            "Идентификатор пользователя (Исполнитель задачи)"
        ):
            check.equal(data_dict["data"][0]["execUserID"], 7886)
        ...
        with check.check, allure.step("Приоритет: срочный, высокий, средний, низкий"):
            check.equal(data_dict["data"][0]["priorityID"], 1)
        with check.check, allure.step("processDate"):
            check.equal(data_dict["data"][0]["processDate"], 1637740620)
        with check.check, allure.step("Метка: Важно, Проверить, Обратить внимание"):
            check.is_none(data_dict["data"][0]["tagID"])
        with check.check, allure.step("Название задачи"):
            check.equal(data_dict["data"][0]["taskName"], "Новая задача")
        with check.check, allure.step("taskNumber"):
            check.equal(data_dict["data"][0]["taskNumber"], 2)
        with check.check, allure.step(
    ...
    def test_change_history_task_37(self, dc_api: DcApiWithToken):
        ...
        with check.check, allure.step("Дата изменения (изменение 1)"):
            check.equal(data_dict["data"][0]["changeDateTime"], 1659015383)
        ...
        with check.check, allure.step("Дата создания (изменение 1)"):
            check.equal(data_dict["data"][0]["createDateTime"], 1659015383)
        with check.check, allure.step(
            "Идентификатор пользователя (Автор задачи) (изменение 1)"
        ):
            check.equal(data_dict["data"][0]["createUserID"], 6305)
        with check.check, allure.step("deleted (изменение 1)"):
            check.is_false(data_dict["data"][0]["deleted"])
        with check.check, allure.step("Описание задачи (изменение 1)"):
            check.equal(
                data_dict["data"][0]["description"], "Операции над учетной записью"
            )
        with check.check, allure.step(
            "Идентификатор пользователя (Исполнитель задачи) (изменение 1)"
        ):
            check.equal(data_dict["data"][0]["execUserID"], 6305)
        ...
        with check.check, allure.step("ID Родителя, если это подзадача (изменение 1)"):
            check.equal(data_dict["data"][0]["parentTaskID"], 0)
        with check.check, allure.step(
            "Приоритет: срочный, высокий, средний, низкий (изменение 1)"
        ):
            check.equal(data_dict["data"][0]["priorityID"], 1)
        with check.check, allure.step("processDate (изменение 1)"):
            check.equal(data_dict["data"][0]["processDate"], 1659015360)
        with check.check, allure.step(
            "Метка: Важно, Проверить, Обратить внимание (изменение 1)"
        ):
            check.is_none(data_dict["data"][0]["tagID"])
        with check.check, allure.step("Название задачи (изменение 1)"):
            check.equal(
                data_dict["data"][0]["taskName"], "Операции над учетной записью"
            )
        with check.check, allure.step("taskNumber (изменение 1)"):
            check.equal(data_dict["data"][0]["taskNumber"], 33)
        with check.check, allure.step(
        ...
        with check.check, allure.step("Дата изменения (изменение 2)"):
            check.equal(data_dict["data"][1]["changeDateTime"], 1659015457)
        with check.check, allure.step("Дата изменения статуса (изменение 2)"):
            check.equal(data_dict["data"][1]["changeStateDateTime"], 1659015457)
        with check.check, allure.step("Срок завершения (изменение 2)"):
            check.is_none(data_dict["data"][1]["completeDateTime"])
        with check.check, allure.step("Дата создания (изменение 2)"):
            check.equal(data_dict["data"][1]["createDateTime"], 1659015383)
        with check.check, allure.step(
            "Идентификатор пользователя (Автор задачи) (изменение 2)"
        ):
            check.equal(data_dict["data"][1]["createUserID"], 6305)
        with check.check, allure.step("deleted (изменение 2)"):
            check.is_false(data_dict["data"][1]["deleted"])
        with check.check, allure.step("Описание задачи (изменение 2)"):
            check.equal(
                data_dict["data"][1]["description"], "Операции над учетной записью"
            )
        with check.check, allure.step(
            "Идентификатор пользователя (Исполнитель задачи) (изменение 2)"
        ):
            check.equal(data_dict["data"][1]["execUserID"], 6305)
        with check.check, allure.step("Пользовательские заметки (изменение 2)"):
            check.is_none(data_dict["data"][1]["notes"])
        with check.check, allure.step("ID Родителя, если это подзадача (изменение 2)"):
            check.equal(data_dict["data"][1]["parentTaskID"], 0)
        with check.check, allure.step(
            "Приоритет: срочный, высокий, средний, низкий (изменение 2)"
        ):
            check.equal(data_dict["data"][1]["priorityID"], 1)
        with check.check, allure.step("processDate (изменение 2)"):
            check.equal(data_dict["data"][1]["processDate"], 1659015480)
        with check.check, allure.step(
            "Метка: Важно, Проверить, Обратить внимание (изменение 2)"
        ):
            check.is_none(data_dict["data"][1]["tagID"])
        with check.check, allure.step("Название задачи (изменение 2)"):
            check.equal(
                data_dict["data"][1]["taskName"], "Операции над учетной записью"
            )
        with check.check, allure.step("taskNumber (изменение 2)"):
            check.equal(data_dict["data"][1]["taskNumber"], 33)
        with check.check, allure.step(
            "Статус: открыта, в работе, выполнено, отклонено (изменение 2)"
        ):
            check.equal(data_dict["data"][1]["taskStateID"], 1)
        with check.check, allure.step(
            "Тип задачи: идентификатор из таблицы TM_TasksTypes (изменение 2)"
        ):
            check.equal(data_dict["data"][1]["taskTypeID"], 1)
