...
class TestAuditorList:
    ...
    def test_REST_test_user_admin_as_auditor(self):
        ...
        with allure.step("UPN"):
            check.equal(
                parsed_data["officers"][2]["UPN"], "REST_test_user_admin@autotest.lan"
            )
        with allure.step("SID"):
            check.equal(
                parsed_data["officers"][2]["SID"],
                "S-1-5-21-4141237049-2453287432-1636914503-3103",
            )
        with allure.step("Почтовый адреса (e-mail)"):
            check.equal(
                parsed_data["officers"][2]["mails"], ["REST_test_user_admin@test.lan"]
            )
        with allure.step("Состояние учетной записи аудитора"):
            check.equal(parsed_data["officers"][2]["Enabled"], True)
        with allure.step("DisplayName"):
            check.equal(
                parsed_data["officers"][2]["DisplayName"], "REST_test_user_admin"
            )
        with allure.step("GUID"):
            check.equal(
                parsed_data["officers"][2]["GUID"],
    ...
    def test_REST_diff_param_as_auditor(self):
        ...
        with allure.step("UPN"):
            check.equal(
                parsed_data["officers"][3]["UPN"], "REST_diff_param@autotest.lan"
            )
        with allure.step("SID"):
            check.equal(
                parsed_data["officers"][3]["SID"],
                "S-1-5-21-4141237049-2453287432-1636914503-3105",
            )
        with allure.step("Почтовый адреса (e-mail)"):
            check.equal(
                parsed_data["officers"][3]["mails"],
                ["REST_test_user_diff_param@test.lan"],
            )
        with allure.step("Состояние учетной записи аудитора"):
            check.equal(parsed_data["officers"][3]["Enabled"], True)
        with allure.step("DisplayName"):
            check.equal(
                parsed_data["officers"][3]["DisplayName"], "REST_test_user_diff_param"
            )
        with allure.step("GUID"):
            check.equal(
                parsed_data["officers"][3]["GUID"],
    ...
    def test_REST_test_user_internal_as_auditor(self):
        ...
        with allure.step("UPN"):
            check.equal(
                parsed_data["officers"][0]["UPN"],
                "REST_test_user_internal@Internal.ISC",
            )
        with allure.step("SID"):
            check.equal(parsed_data["officers"][0]["SID"], "")
        with allure.step("Почтовый адреса (e-mail)"):
            check.equal(parsed_data["officers"][0]["mails"], [])
        with allure.step("Состояние учетной записи аудитора"):
            check.equal(parsed_data["officers"][0]["Enabled"], True)
        with allure.step("DisplayName"):
            check.equal(
                parsed_data["officers"][0]["DisplayName"], "REST_test_user_internal"
            )
        with allure.step("GUID"):
            check.equal(
                parsed_data["officers"][0]["GUID"],
