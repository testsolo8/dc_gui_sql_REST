...
class TestConnectionParametersToDCserver:
    ...
    def test_connection_parameters_to_DCserver(self):
        ...
        with allure.step("Порт APIv3"):
            check.equal(data_dict[0]["Ports"]["APIv3"], 9082)
        with allure.step("Порт SIProxyDC"):
            check.equal(data_dict[0]["Ports"]["SIProxyDC"], 9088)
        with allure.step("Порт UserCard"):
            check.equal(data_dict[0]["Ports"]["UserCardDS"], 9072)
        with allure.step("UID сервера"):
            check.equal(data_dict[0]["UID"], "{124645B2-4B96-4675-868A-1E1958018B42}")
        with allure.step("HostName сервера"):
            check.is_in("AUTOTEST", data_dict[0]["HostName"])
        with allure.step("Версия сервера"):
            check.greater(data_dict[0]["Version"], "2.50.0.0")
