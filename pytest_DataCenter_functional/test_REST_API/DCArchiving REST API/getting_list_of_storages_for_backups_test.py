...
class TestGettingListOfStoragesForBackups:
    ...
    def test_db_migration_storage(self):
        ...
        with allure.step("Версия API агента БД. Если не опознано - вернется 0"):
            check.equal(data_dict[0]["AgentAPIVersion"], 2)
        with allure.step("Порт службы агента БД."):
            check.equal(data_dict[0]["AgentAPIPort"], 9085)
        with allure.step("Флаг необходимости обновить агент"):
            check.is_false(data_dict[0]["AgentNeedsUpdate"])
        with allure.step("Mode"):
            check.equal(data_dict[0]["Mode"], 0)
        with allure.step("Лимит на шару в мегабайтах"):
            check.equal(data_dict[0]["Limit"], 0)
    ...
    def test_index_migration_storage(self):
        ...
        with allure.step("ID хранилища"):
            check.equal(data_dict[1]["ID"], "6b6c02ecb70a888ba5ea5dd81e6b41bc")
        with allure.step("Путь до целевой папки на шаре"):
            check.equal(data_dict[1]["Path"], "d:\index")
        with allure.step(
            "Имя пользователя для авторизации (если отсутствует - имперсонация выполняться не будет)"
        ):
            check.equal(data_dict[1]["User"], "")
        with allure.step(
            "Пароль для пользователя заданного в ShareUser. Шифрование стандартное для наших строк"
        ):
            check.equal(data_dict[1]["Password"], "")
        with allure.step("Имя сервера SQL"):
            check.equal(data_dict[1]["Server"], "nsa2.minsk.searchinform.net")
        with allure.step("Версия агента БД. Если не опознано - вернется Unknown"):
            agent_version = list(data_dict[1]["AgentVersion"].split("."))
            with soft_assertions():
                assert_that(agent_version).is_not_empty()
                assert_that(agent_version).is_length(4)
                assert_that(agent_version[0]).is_equal_to("2")
        with allure.step("Версия API агента БД. Если не опознано - вернется 0"):
            check.equal(data_dict[1]["AgentAPIVersion"], 2)
        with allure.step("Порт службы агента БД."):
            check.equal(data_dict[1]["AgentAPIPort"], 9085)
        with allure.step("Флаг необходимости обновить агент"):
            check.is_false(data_dict[1]["AgentNeedsUpdate"])
        with allure.step("Mode"):
            check.equal(data_dict[1]["Mode"], 0)
        with allure.step("Лимит на шару в мегабайтах"):
            check.equal(data_dict[1]["Limit"], 0)
    ...
    def test_index_backup_storage(self):
        ...
        with allure.step("ID хранилища"):
            check.equal(data_dict[2]["ID"], "8ea14f593b39fcccb1c67df20db9220a")
        with allure.step("Путь до целевой папки на шаре"):
            check.equal(data_dict[2]["Path"], r"\\msq-nsa-sql\dataoldf$\backup\index")
        with allure.step(
            "Имя пользователя для авторизации (если отсутствует - имперсонация выполняться не будет)"
        ):
            check.equal(data_dict[2]["User"], "")
        with allure.step(
            "Пароль для пользователя заданного в ShareUser. Шифрование стандартное для наших строк"
        ):
            check.equal(data_dict[2]["Password"], "")
        with allure.step("Имя сервера SQL"):
            check.equal(data_dict[2]["Server"], "nsa2.minsk.searchinform.net")
        with allure.step("Версия агента БД. Если не опознано - вернется Unknown"):
            agent_version = list(data_dict[2]["AgentVersion"].split("."))
            with soft_assertions():
                assert_that(agent_version).is_not_empty()
                assert_that(agent_version).is_length(4)
                assert_that(agent_version[0]).is_equal_to("2")
        with allure.step("Версия API агента БД. Если не опознано - вернется 0"):
            check.equal(data_dict[2]["AgentAPIVersion"], 2)
        with allure.step("Порт службы агента БД."):
            check.equal(data_dict[2]["AgentAPIPort"], 9085)
        with allure.step("Флаг необходимости обновить агент"):
            check.is_false(data_dict[2]["AgentNeedsUpdate"])
        with allure.step("Mode"):
            check.equal(data_dict[2]["Mode"], 0)
        with allure.step("Лимит на шару в мегабайтах"):
            check.equal(data_dict[2]["Limit"], 0)
    ...
    def test_sti_backup_storage(self):
        ...
        with allure.step("ID хранилища"):
            check.equal(data_dict[3]["ID"], "a0c980e71a704281f392ac2f3df9978b")
        with allure.step("Путь до целевой папки на шаре"):
            check.equal(data_dict[3]["Path"], r"\\msq-nsa-sql\dataoldf$\backup\sti")
        with allure.step(
            "Имя пользователя для авторизации (если отсутствует - имперсонация выполняться не будет)"
        ):
            check.equal(data_dict[3]["User"], "")
        with allure.step(
            "Пароль для пользователя заданного в ShareUser. Шифрование стандартное для наших строк"
        ):
            check.equal(data_dict[3]["Password"], "")
        with allure.step("Имя сервера SQL"):
            check.equal(data_dict[3]["Server"], "nsa2.minsk.searchinform.net")
        with allure.step("Версия агента БД. Если не опознано - вернется Unknown"):
            agent_version = list(data_dict[3]["AgentVersion"].split("."))
            with soft_assertions():
                assert_that(agent_version).is_not_empty()
                assert_that(agent_version).is_length(4)
                assert_that(agent_version[0]).is_equal_to("2")
        with allure.step("Версия API агента БД. Если не опознано - вернется 0"):
            check.equal(data_dict[3]["AgentAPIVersion"], 2)
        with allure.step("Порт службы агента БД."):
            check.equal(data_dict[3]["AgentAPIPort"], 9085)
        with allure.step("Флаг необходимости обновить агент"):
            check.is_false(data_dict[3]["AgentNeedsUpdate"])
        with allure.step("Mode"):
            check.equal(data_dict[3]["Mode"], 0)
        with allure.step("Лимит на шару в мегабайтах"):
            check.equal(data_dict[3]["Limit"], 0)
    ...
    def test_db_backup_storage(self):
        ...
        with allure.step("ID хранилища"):
            check.equal(data_dict[4]["ID"], "bb8271d84f55d989a044d206411f706a")
        with allure.step("Путь до целевой папки на шаре"):
            check.equal(data_dict[4]["Path"], r"\\msq-nsa-sql\dataoldf$\backup\db")
        with allure.step(
            "Имя пользователя для авторизации (если отсутствует - имперсонация выполняться не будет)"
        ):
            check.equal(data_dict[4]["User"], "")
        with allure.step(
            "Пароль для пользователя заданного в ShareUser. Шифрование стандартное для наших строк"
        ):
            check.equal(data_dict[4]["Password"], "")
        with allure.step("Имя сервера SQL"):
            check.equal(data_dict[4]["Server"], "TAKE-SQL.minsk.searchinform.net")
        with allure.step("Версия агента БД. Если не опознано - вернется Unknown"):
            agent_version = list(data_dict[4]["AgentVersion"].split("."))
            with soft_assertions():
                assert_that(agent_version).is_not_empty()
                assert_that(agent_version).is_length(4)
                assert_that(agent_version[0]).is_equal_to("2")
        with allure.step("Версия API агента БД. Если не опознано - вернется 0"):
            check.equal(data_dict[4]["AgentAPIVersion"], 2)
        with allure.step("Порт службы агента БД."):
            check.equal(data_dict[4]["AgentAPIPort"], 9085)
        with allure.step("Флаг необходимости обновить агент"):
            check.is_false(data_dict[4]["AgentNeedsUpdate"])
        with allure.step("Mode"):
            check.equal(data_dict[4]["Mode"], 0)
        with allure.step("Лимит на шару в мегабайтах"):
            check.equal(data_dict[4]["Limit"], 0)
    ...
    def test_sti_migration_storage(self):
        ...
        with allure.step("ID хранилища"):
            check.equal(data_dict[5]["ID"], "bda31da73f1f454e2f5305aaba9015b3")
        with allure.step("Путь до целевой папки на шаре"):
            check.equal(data_dict[5]["Path"], r"d:\sti")
        with allure.step(
            "Имя пользователя для авторизации (если отсутствует - имперсонация выполняться не будет)"
        ):
            check.equal(data_dict[5]["User"], "")
        with allure.step(
            "Пароль для пользователя заданного в ShareUser. Шифрование стандартное для наших строк"
        ):
            check.equal(data_dict[5]["Password"], "")
        with allure.step("Имя сервера SQL"):
            check.equal(data_dict[5]["Server"], "nsa2.minsk.searchinform.net")
        with allure.step("Версия агента БД. Если не опознано - вернется Unknown"):
            agent_version = list(data_dict[5]["AgentVersion"].split("."))
            with soft_assertions():
                assert_that(agent_version).is_not_empty()
                assert_that(agent_version).is_length(4)
                assert_that(agent_version[0]).is_equal_to("2")
        with allure.step("Версия API агента БД. Если не опознано - вернется 0"):
            check.equal(data_dict[5]["AgentAPIVersion"], 2)
        with allure.step("Порт службы агента БД."):
            check.equal(data_dict[5]["AgentAPIPort"], 9085)
        with allure.step("Флаг необходимости обновить агент"):
            check.is_false(data_dict[5]["AgentNeedsUpdate"])
        with allure.step("Mode"):
            check.equal(data_dict[5]["Mode"], 0)
        with allure.step("Лимит на шару в мегабайтах"):
            check.equal(data_dict[5]["Limit"], 0)
