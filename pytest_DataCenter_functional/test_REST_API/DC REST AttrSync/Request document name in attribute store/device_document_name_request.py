...
@allure.story("Запрос имени документа в хранилище атрибутов по индексу Device")
class TestRequestDocumentNameInAttributeStoreByIndexes:
    ...
    def test_return_data(self):
        data = [
            "$data",
            "kolontaev_kratkaya-istoriya-morskogo-specnaza-ukrainy_rulit_me_541193 (1).doc",
            "лермонтов_4.txt",
            "156 стр - психологические основы обучения воинов сухопутных воиск (1).docx",
        ]
        data_dict = r.json()
        data_str = json.dumps(data_dict)
        parsed_data = json.loads(data_str)
