...
test_ids = ["{}, {}, {}, {}".format(t[0], t[1], t[2], t[3]) for t in testdata_auditor_spec]

@pytest.mark.order(130)
@allure.epic("Auditor test")
@allure.feature("Create auditor 'spec' on MSSQL")
@pytest.mark.testAuditor
@pytest.mark.parametrize(argnames="db_data", argvalues=testdata_auditor_spec, ids=test_ids)
def test_add_auditor_spec_to_mssql_db(save_and_repair_main_db_name, start_test_page_for_auditor, db_data):
    with allure.step(
        "Время начала теста "
        + "("
    ...
    try:
        ...
        with pyodbc.connect(connectionStr, autocommit=True) as cursor:
            sqlMask = (
                f"DECLARE @id int = (SELECT TOP(1) obj_id FROM AD_ObjectAttributes WHERE userPrincipalName = 'spec@Internal.ISC');"
