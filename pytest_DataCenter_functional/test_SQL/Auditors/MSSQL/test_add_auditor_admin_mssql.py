...
test_ids = ["{}, {}, {}, {}".format(t[0], t[1], t[2], t[3]) for t in testdata_auditor_admin]

@pytest.mark.order(130)
@allure.epic("Auditor test")
@allure.feature("Create auditor 'admin' on MSSQL")
@pytest.mark.testAuditor
@pytest.mark.parametrize(argnames="db_data", argvalues=testdata_auditor_admin, ids=test_ids)
def test_add_auditor_admin_to_mssql_db(save_and_repair_main_db_name, start_test_page_for_auditor, db_data):
    with allure.step(
        "Время начала теста "
        + "("
