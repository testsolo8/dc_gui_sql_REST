...
from pytest_DataCenter_functional.test_SQL.Auditors.auditors_tools import (
        add_admin_auditor, create_internal_user_admin
)

testdata_auditor_admin = [
    (
        "192.168.1.12",
        "5413",
        "postgres",
        "Passw0rd",
        "datacenter_for_auditor_admin",
...
test_ids = ["{}, {}, {}, {}".format(t[0], t[1], t[2], t[4]) for t in testdata_auditor_admin]


@pytest.mark.order(130)
@allure.epic("Auditor test")
@allure.feature("Create auditor 'admin' on PostgreSQL")
@pytest.mark.testAuditor
@pytest.mark.parametrize(argnames="db_data", argvalues=testdata_auditor_admin, ids=test_ids)
def test_add_auditor_admin_to_postgresql_db(save_and_repair_main_db_name, start_test_page_for_auditor, db_data):
    with allure.step(
            "Время начала теста "
            + "("
    ...
    time.sleep(1)
    main_dlg.__getattribute__("Edit3").type_keys("admin")  # Login
    time.sleep(1)
    main_dlg.__getattribute__("Edit4").type_keys(
        "{VK_SHIFT down}" "p" "{VK_SHIFT up}" "assw0rd")  # Password
    time.sleep(1)
    ...
    try:
        sqlMask = (f"SELECT obj_id FROM ad_objectattributes WHERE userprincipalname = 'admin@Internal.ISC' LIMIT 1;")
