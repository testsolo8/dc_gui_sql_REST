...
from pytest_DataCenter_functional.test_SQL.Auditors.auditors_tools import (
         create_internal_cyrillic_user_admin, add_cyrillic_admin_auditor,
)
...
test_ids = ["{}, {}, {}, {}".format(t[0], t[1], t[2], t[4]) for t in testdata_cyrillic_auditor_admin]

@pytest.mark.order(130)
@allure.epic("Auditor test")
@allure.feature("Create auditor 'админ' on PostgreSQL")
@pytest.mark.testAuditor
@pytest.mark.parametrize(argnames="db_data", argvalues=testdata_cyrillic_auditor_admin, ids=test_ids)
def test_add_cyrillic_auditor_admin_to_postgresql_db(save_and_repair_main_db_name, start_test_page_for_auditor, db_data):
    with allure.step(
            "Время начала теста "
            + "("
    ...
    try:
        sqlMask = (f"SELECT obj_id FROM ad_objectattributes WHERE userprincipalname = 'админ@Internal.ISC' LIMIT 1;")
