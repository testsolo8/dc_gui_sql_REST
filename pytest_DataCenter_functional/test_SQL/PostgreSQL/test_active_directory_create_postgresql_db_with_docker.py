    ...
    def test_create_active_directory_postgresql_db(self, handle, db_data):
        ...
        if exist_db is True:
            ...
            with check.check:
                ...
                ).is_equal_to(set(dc_active_directory_postgresql))
        delete_postgres_DB(sql_serv_addr, port, username, password, db_name)
...
class TestCreatePostgreSQLActiveDirectoryDBWithSpecSymbol:
    @pytest.mark.parametrize(
        argnames="db_data_spec", argvalues=testdata_spec_symbol, ids=test_ids_spec
    )
    def test_create_active_directory_postgresql_db_with_spec_symbols(
