...
class TestCreatePostgreSQLTaskManagementDB:
    ...
    def test_create_task_management_postgresql_db(self, handle, db_data):
        ...
        while time.time() - start_time < 120:
            try:
                ...
                if not exist_db:
                    ...
                    logging.info("БД TaskManagement создана")
                    break
        ...
        with check.check:
            ...
            ).is_equal_to(task_management_postgresql)
        delete_postgres_DB(sql_serv_addr, port, username, password, db_name)
...
class TestCreatePostgreSQLTaskManagementDBWithSpecSymbol:
    @pytest.mark.parametrize(
        argnames="db_data_spec", argvalues=testdata_spec_symbol, ids=test_ids_spec
    )
    def test_create_task_management_postgresql_db_with_spec_symbols(
