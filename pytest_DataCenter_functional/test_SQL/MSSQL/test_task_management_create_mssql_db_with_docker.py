...
# My packages
from DataCenter.buttons import TaskManagementPage, CommonElements
from pytest_DataCenter_functional.test_SQL.sql_tools import Docker, delete_mssql_DB
from pytest_DataCenter_functional.test_SQL.tables_templates import task_management_mssql
...
class TestCreateMSSQLTaskManagementDB:
    ...
    def test_create_task_managemet_db(self, handle, db_data):
        ...
        time.sleep(2)
        handle.by(name=TaskManagementPage.read_from_dc["title_ru"]).click()
        time.sleep(2)
        handle_win32.__getattribute__("ComboBox2").type_keys(sql_serv_addr)
        time.sleep(2)
        handle.by(name=TaskManagementPage.create_button["title_ru"]).click()
        time.sleep(5)
        handle.by(name=CommonElements.ok_button["title_ru"]).__getattribute__(
            CommonElements.ok_button["attribute_ru"]
        ).click()
        time.sleep(1)
        handle.by(name=TaskManagementPage.ok_button["title_ru"]).click()
        time.sleep(2)
        try:
            handle_win32.close()
        ...
        time.sleep(2)
        handle.by(name=TaskManagementPage.apply_button["title_ru"]).click()
            ...
            try:
                ...
                with pyodbc.connect(connectionStr, autocommit=True) as cursor:
                    ...
                    if not rows:
                        ...
                        logging.info("БД Task Management создана")
                        break
            ...
            while time.time() - start_time < 120:
                try:
                    ...
                    if not list_of_tables:
                        time.sleep(10)
                        continue
                    if len(list_of_tables) < 17:
                        time.sleep(10)
                    else:
                        logging.info("Корректное кол-во таблиц в БД Task Management")
                        break
                except Exception as ex:
                    logging.debug(ex)
            with check.check:
                ...
                ).is_equal_to(task_management_mssql)
        delete_mssql_DB(sql_serv_addr, username, password, db_name)
...
class TestCreateMSSQLTaskManagementDBWithSpecSymbol:
    @pytest.mark.parametrize(
        argnames="db_data_spec", argvalues=testdata_spec_symbol, ids=test_ids_spec
    )
    def test_create_task_managemet_db_with_spec_symbols(
        ...
        handle_with_spec_symbol.by(
            name=TaskManagementPage.setup_connection_to_db["title_ru"]
        ).click()
        time.sleep(2)
        handle_win32.__getattribute__("ComboBox2").type_keys("^a{BACKSPACE}")
        time.sleep(2)
        ...
        time.sleep(2)
        handle_with_spec_symbol.by(
            name=TaskManagementPage.apply_button["title_ru"]
        ).click()
        start_time = time.time()
        while time.time() - start_time < 120:
            try:
