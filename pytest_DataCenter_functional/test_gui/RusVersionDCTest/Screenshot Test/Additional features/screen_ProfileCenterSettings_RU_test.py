...
def handle_sql_server_connection_setting(start_screen_test_page_ru):
...
@pytest.fixture(scope="class")
def schedule_of_reading_window(start_screen_test_page_ru):
    send_keys("{VK_MENU down}Y4Y11{VK_MENU up}")
    app = start_screen_test_page_ru
    handle = app.window(name_re="DataCenter*")
...
@pytest.fixture(scope="class")
def schedule_of_reading_second_window(start_screen_test_page_ru):
    send_keys("{VK_MENU down}Y4Y11{VK_MENU up}")
...
@pytest.fixture(scope="class")
def schedule_of_reading_third_window(start_screen_test_page_ru):
    send_keys("{VK_MENU down}Y4Y11{VK_MENU up}")
    app = Application(backend="uia").connect(
        path=r"c:\Program Files (x86)\SearchInform\SearchInform DataCenter\DCClient.exe"
    )
...
@pytest.fixture(scope="class")
def schedule_of_profiling_window(start_screen_test_page_ru):
    send_keys("{VK_MENU down}Y4Y11{VK_MENU up}")
    app = start_screen_test_page_ru
    handle = app.window(name_re="DataCenter*")
    handle.__getattribute__(
...
@pytest.fixture(scope="class")
def schedule_of_profiling_second_window(start_screen_test_page_ru):
    send_keys("{VK_MENU down}Y4Y11{VK_MENU up}")
    app = Application(backend="uia").connect(
        path=r"c:\Program Files (x86)\SearchInform\SearchInform DataCenter\DCClient.exe"
    )
...
@pytest.fixture(scope="class")
def schedule_of_profiling_third_window(start_screen_test_page_ru):
    send_keys("{VK_MENU down}Y4Y11{VK_MENU up}")
    app = Application(backend="uia").connect(
        path=r"c:\Program Files (x86)\SearchInform\SearchInform DataCenter\DCClient.exe"
    )
...
class TestProfileCenterSettingsPage:
    ...
    class TestProfileCenterSettingsMainPage:
        def test_profile_center_settings_page_screen(self, start_screen_test_page_ru):
            send_keys("{VK_MENU down}Y4Y11{VK_MENU up}")
            time.sleep(2)
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsRU.profile_center_settings["path"],
    ...
    class TestSQLServerConnectionSettingWindow:
        def test_sql_server_connection_setting_screen(
            ...
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsRU.profile_center_settings_sql_server_connection_setting_window[
                    "path"
                ],
                confidence=LevelOfConfidence.confidence["confidence"],
            )
    ...
    class TestScheduleOfReadingWindow:
        def test_schedule_of_reading_window(self, schedule_of_reading_window):
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsRU.profile_schedule_of_reading_window["path"],
                confidence=LevelOfConfidence.confidence["confidence"],
            )
            make_screenshot_if_error(check.is_not_none(screenshot))
    ...
    class TestScheduleOfReadingSecondWindow:
        def test_schedule_of_reading_second_window(
            ...
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsRU.profile_schedule_of_reading_second_window["path"],
                confidence=LevelOfConfidence.confidence["confidence"],
            )
            make_screenshot_if_error(check.is_not_none(screenshot))
    ...
    class TestScheduleOfReadingThirdWindow:
        def test_schedule_of_reading_third_window(
            ...
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsRU.profile_schedule_of_reading_third_window["path"],
                confidence=LevelOfConfidence.confidence["confidence"],
            )
            make_screenshot_if_error(check.is_not_none(screenshot))
    ...
    class TestScheduleOfProfilingWindow:
        def test_schedule_of_profiling_window(self, schedule_of_profiling_window):
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsRU.profile_schedule_of_profiling_window["path"],
                confidence=LevelOfConfidence.confidence["confidence"],
            )
            make_screenshot_if_error(check.is_not_none(screenshot))
    ...
    class TestScheduleOfProfilingSecondWindow:
        def test_schedule_of_profiling_second_window(
            ...
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsRU.profile_schedule_of_profiling_second_window["path"],
                confidence=LevelOfConfidence.confidence["confidence"],
            )
            make_screenshot_if_error(check.is_not_none(screenshot))
    ...
    class TestScheduleOfProfilingThirdWindow:
        def test_schedule_of_profiling_third_window(
            ...
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsRU.profile_schedule_of_profiling_third_window["path"],
                confidence=LevelOfConfidence.confidence["confidence"],
            )
            make_screenshot_if_error(check.is_not_none(screenshot))
