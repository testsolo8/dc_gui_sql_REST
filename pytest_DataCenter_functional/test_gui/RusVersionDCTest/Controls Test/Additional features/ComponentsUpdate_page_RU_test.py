...
class TestComponentsUpdatePage:
    def test_cancel_button(self, handle):
        cancel_button = (
            handle.by(name=ComponentsUpdatePage.cancel_button["title_ru"])
            .find()
            .window_text()
        )
    ...
    def test_apply_button(self, handle):
        apply_button = (
            handle.by(name=ComponentsUpdatePage.apply_button["title_ru"])
            .find()
            .window_text()
        )
    ...
    def test_analyticconsole(self, handle):
        analyticconsole = (
            handle.by(name=ComponentsUpdatePage.analyticconsole["title_ru"])
            .find()
            .window_text()
        )
    ...
    def test_automatic_update_of_versions3(self, handle):
        automatic_update_of_versions3 = (
            handle.by(
                name=ComponentsUpdatePage.automatic_update_of_versions3["title_ru"],
        ...
        check.equal(automatic_update_of_versions3, "Автообновление версий")

    def test_installation_in_silent_mode(self, handle):
        installation_in_silent_mode = (
            handle.by(name=ComponentsUpdatePage.installation_in_silent_mode["title_ru"])
            .find()
            .window_text()
        )
        check.equal(installation_in_silent_mode, "Установка в тихом режиме")

    def test_api_installation(self, handle):
        api_installation = (
            handle.by(name=ComponentsUpdatePage.api_installation["title_ru"])
            .find()
            .window_text()
        )
        check.equal(api_installation, "Установка API")

    def test_version3(self, handle):
        version3 = (
            handle.by(
                name=ComponentsUpdatePage.version3["title_ru"],
                found_index=ComponentsUpdatePage.version3["found_index"],
            )
            .find()
            .window_text()
        )
        check.equal(version3, "Версия:")

    def test_reportcenter(self, handle):
        reportcenter = (
            handle.by(name=ComponentsUpdatePage.reportcenter["title_ru"])
            .find()
            .window_text()
        )
    ...
    def test_automatic_update_of_versions2(self, handle):
        automatic_update_of_versions2 = (
            handle.by(
                name=ComponentsUpdatePage.automatic_update_of_versions2["title_ru"],
                found_index=ComponentsUpdatePage.automatic_update_of_versions3[
                    "found_index"
                ],
            )
        ...
        check.equal(automatic_update_of_versions2, "Автообновление версий")

    def test_version2(self, handle):
        version2 = (
            handle.by(
                name=ComponentsUpdatePage.version2["title_ru"],
                found_index=ComponentsUpdatePage.version2["found_index"],
            )
            .find()
            .window_text()
        )
        check.equal(version2, "Версия:")

    def test_searchapi(self, handle):
        searchapi = (
            handle.by(name=ComponentsUpdatePage.searchapi["title_ru"])
            .find()
            .window_text()
        )
    ...
    def test_automatic_update_of_versions(self, handle):
        automatic_update_of_versions = (
            handle.by(
                name=ComponentsUpdatePage.automatic_update_of_versions["title_ru"],
        ...
        check.equal(automatic_update_of_versions, "Автообновление версий")

    def test_version(self, handle):
        version = (
            handle.by(
                name=ComponentsUpdatePage.version["title_ru"],
                found_index=ComponentsUpdatePage.version["found_index"],
            )
            .find()
            .window_text()
        )
        check.equal(version, "Версия:")

    def test_webanalytic(self, handle):
        webanalytic = (
            handle.by(name=ComponentsUpdatePage.webanalytic["title_ru"])
            .find()
            .window_text()
        )
    ...
    def test_automatic_update_of_versions4(self, handle):
        automatic_update_of_versions4 = (
            handle.by(
                name=ComponentsUpdatePage.automatic_update_of_versions4["title_ru"],
                found_index=ComponentsUpdatePage.automatic_update_of_versions[
                    "found_index"
                ],
            )
        ...
        check.equal(automatic_update_of_versions4, "Автообновление версий")

    def test_version4(self, handle):
        version4 = (
            handle.by(
                name=ComponentsUpdatePage.version4["title_ru"],
                found_index=ComponentsUpdatePage.version4["found_index"],
            )
            .find()
            .window_text()
        )
        check.equal(version4, "Версия:")
