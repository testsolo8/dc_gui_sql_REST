...
@pytest.fixture(scope="class")
def handle_free_disk_space_monitoring_window(start_fast_test_page_ru):
    send_keys("{VK_MENU down}Y3Y3{VK_MENU up}")
    app = start_fast_test_page_ru
    handle = app.window(name_re="DataCenter*")
...
@pytest.fixture(scope="class")
def handle_change_mail_button(start_fast_test_page_ru):
    send_keys("{VK_MENU down}Y3Y3{VK_MENU up}")
    app = start_fast_test_page_ru
    handle = app.window(name_re="DataCenter*")
...
class TestAgentsAndComponentsPage:
    ...
    class TestAgentsAndComponentsMainPage:
        def test_cancel_button(self, handle):
            cancel_button = (
                handle.by(name=AgentsAndComponentsPage.cancel_button["title_ru"])
                .find()
                .window_text()
            )
        ...
        def test_apply_button(self, handle):
            apply_button = (
                handle.by(name=AgentsAndComponentsPage.apply_button["title_ru"])
                .find()
                .window_text()
            )
    ...
    class TestAgentsAndComponentsFreeDiskSpaceMonitoringWindow:
        ...
        def test_enable_monitoring(self, handle_free_disk_space_monitoring_window):
            enable_monitoring = (
                handle_free_disk_space_monitoring_window.by(
                    name=AgentsAndComponentsPage.enable_monitoring["title_ru"],
            ...
            check.equal(enable_monitoring, "Включить мониторинг")

        def test_cancel2(self, handle_free_disk_space_monitoring_window):
            cancel2 = (
                handle_free_disk_space_monitoring_window.by(
                    name=AgentsAndComponentsPage.cancel2["title_ru"],
                    found_index=AgentsAndComponentsPage.enable_monitoring[
                        "found_index"
                    ],
                )
        ...
        def test_ok_button(self, handle_free_disk_space_monitoring_window):
            ok_button = (
                handle_free_disk_space_monitoring_window.by(
                    name=AgentsAndComponentsPage.ok_button["title_ru"]
                )
                .find()
                .window_text()
            )
    ...
    class TestAgentsAndComponentsChangeMailWindow:
        def test_each_user_mustbe_specified_in_new_line(
            ...
            check.equal(
                each_user_mustbe_specified_in_new_line,
                "* Каждый пользователь должен быть указан с новой строки",
            )
        ...
        def test_list_of_telegram_users_authorized_enabledisable_notifications(
            ...
            check.equal(
                list_of_telegram_users_authorized_enabledisable_notifications,
                "Список пользователей Telegram, которым разрешено подключать/отключать уведомления:",
            )

        def test_settings_socks5proxy(self, handle_change_mail_button):
            settings_socks5proxy = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.settings_socks5proxy["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_password(self, handle_change_mail_button):
            password = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.password["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_username(self, handle_change_mail_button):
            username = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.username["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_server_port(self, handle_change_mail_button):
            server_port = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.server_port["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_server(self, handle_change_mail_button):
            server = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.server["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_use_proxy(self, handle_change_mail_button):
            use_proxy = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.use_proxy["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_telegram_bot_token(self, handle_change_mail_button):
            telegram_bot_token = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.telegram_bot_token["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_redirect_to_another_dcagent(self, handle_change_mail_button):
            redirect_to_another_dcagent = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.redirect_to_another_dcagent["title_ru"]
                )
                .find()
                .window_text()
            )
            check.equal(redirect_to_another_dcagent, "Перенаправить на другой DCAgent")

        def test_customize_bot(self, handle_change_mail_button):
            customize_bot = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.customize_bot["title_ru"]
                )
                .find()
                .window_text()
            )
            check.equal(customize_bot, "Персональная настройка бота")
        ...
        def test_use_following_settings_send_notifications_telegram(
            ...
            check.equal(
                use_following_settings_send_notifications_telegram,
                "Использовать следующие настройки для отправки уведомлений в Telegram:",
            )

        def test_close(self, handle_change_mail_button):
            close = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.close["title_ru"],
                    found_index=AgentsAndComponentsPage.close["found_index"],
                )
                .find()
                .window_text()
            )
            check.equal(close, "Закрыть")

        def test_save(self, handle_change_mail_button):
            save = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.save["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_check_connection(self, handle_change_mail_button):
            check_connection = (
                handle_change_mail_button.__getattribute__(
                    AgentsAndComponentsPage.check_connection["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_check_connection2(self, handle_change_mail_button):
            check_connection = (
                handle_change_mail_button.__getattribute__(
                    AgentsAndComponentsPage.check_connection2["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_authentication(self, handle_change_mail_button):
            authentication = (
                handle_change_mail_button.by(
                    name=AgentsAndComponentsPage.authentication["title_ru"]
                )
                .find()
                .window_text()
            )
        ...
        def test_use_following_settings_send_email_notifications(
            ...
            check.equal(
                use_following_settings_send_email_notifications,
                "Использовать следующие настройки для отправки почтовых уведомлений:",
            )
