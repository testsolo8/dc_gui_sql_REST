...
def new_migration_db_rule_window(start_screen_test_page_es):
...
@pytest.fixture(scope="class")
def new_backup_db_rule_window(start_screen_test_page_es):
    send_keys("{VK_MENU down}Y4Y05{VK_MENU up}")
    app = start_screen_test_page_es
    handle = app.window(name_re="DataCenter*")
    handle.by(name=DBBackupAndMigrationPage.new_rule["title_es"]).click()
...
@pytest.fixture(scope="class")
def new_migration_index_rule_window(start_screen_test_page_es):
    send_keys("{VK_MENU down}Y4Y05{VK_MENU up}")
    app = start_screen_test_page_es
    handle = app.window(name_re="DataCenter*")
    handle.by(name=DBBackupAndMigrationPage.new_rule["title_es"]).click()
...
@pytest.fixture(scope="class")
def new_backup_index_rule_window(start_screen_test_page_es):
    send_keys("{VK_MENU down}Y4Y05{VK_MENU up}")
    app = start_screen_test_page_es
    handle = app.window(name_re="DataCenter*")
    handle.by(name=DBBackupAndMigrationPage.new_rule["title_es"]).click()
...
@pytest.fixture(scope="class")
def new_migration_storage_rule_window(start_screen_test_page_es):
    send_keys("{VK_MENU down}Y4Y05{VK_MENU up}")
    app = start_screen_test_page_es
    handle = app.window(name_re="DataCenter*")
    handle.by(name=DBBackupAndMigrationPage.new_rule["title_es"]).click()
...
@pytest.fixture(scope="class")
def new_backup_storage_rule_window(start_screen_test_page_es):
    send_keys("{VK_MENU down}Y4Y05{VK_MENU up}")
    app = start_screen_test_page_es
    handle = app.window(name_re="DataCenter*")
    handle.by(name=DBBackupAndMigrationPage.new_rule["title_es"]).click()
...
class TestDBBackupAndMigrationPage:
    ...
    class TestDBBackupAndMigrationMainPage:
        def test_db_backup_and_migration_page_screen(self, start_screen_test_page_es):
            send_keys("{VK_MENU down}Y4Y05{VK_MENU up}")
            time.sleep(2)
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.db_backup_and_migration["path"],
    ...
    class TestAddNewMigrationDBRuleWindow:
        def test_new_migration_db_rule_window(self, new_migration_db_rule_window):
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.db_backup_and_migration_add_new_migration_db_rule_window[
                    "path"
                ],
                # confidence=LevelOfConfidence.confidence["confidence"],
            )
    ...
    class TestAddNewBackupDBRuleWindow:
        def test_new_backup_db_rule_window(self, new_backup_db_rule_window):
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.db_backup_and_migration_add_new_backup_db_rule_window[
                    "path"
                ],
                # confidence=LevelOfConfidence.confidence["confidence"],
            )
    ...
    class TestAddNewMigrationIndexRuleWindow:
        def test_new_migration_index_rule_window(self, new_migration_index_rule_window):
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.db_backup_and_migration_add_new_migration_index_rule_window[
                    "path"
                ],
                # confidence=LevelOfConfidence.confidence["confidence"],
            )
    ...
    class TestAddNewBackupIndexRuleWindow:
        def test_new_backup_index_rule_window(self, new_backup_index_rule_window):
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.db_backup_and_migration_add_new_backup_index_rule_window[
                    "path"
                ],
                # confidence=LevelOfConfidence.confidence["confidence"],
            )
    ...
    class TestAddNewMigrationStorageRuleWindow:
        def test_new_migration_storage_rule_window(
            ...
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.db_backup_and_migration_add_new_migration_storage_rule_window[
                    "path"
                ],
                # confidence=LevelOfConfidence.confidence["confidence"],
            )
    ...
    class TestAddNewBackupStorageRuleWindow:
        def test_new_backup_storage_rule_window(self, new_backup_storage_rule_window):
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.db_backup_and_migration_add_new_backup_storage_rule_window[
                    "path"
                ],
                # confidence=LevelOfConfidence.confidence["confidence"],
            )
