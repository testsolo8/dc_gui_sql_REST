...
def handle_free_disk_space_monitoring_window(start_screen_test_page_es):
...
@pytest.fixture(scope="class")
def handle_change_mail_button(start_screen_test_page_es):
    send_keys("{VK_MENU down}Y3Y3{VK_MENU up}")
    app = start_screen_test_page_es
    handle = app.window(name_re="DataCenter*")
...
class TestAgentsAndComponentsPage:
    ...
    class TestAgentsAndComponentsMainPage:
        def test_agents_and_components_screen(self, start_screen_test_page_es):
            send_keys("{VK_MENU down}Y3Y3{VK_MENU up}")
            time.sleep(2)
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.agents_and_components["path"],
    ...
    class TestAgentsAndComponentsFreeDiskSpaceMonitoringWindow:
        def test_free_disk_space_monitoring_window(
            ...
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.agents_and_components_free_disk_space_monitoring_window[
                    "path"
                ],
    ...
    class TestAgentsAndComponentsChangeMailWindow:
        def test_change_mail_button(self, handle_change_mail_button):
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.agents_and_components_change_mail_window["path"],
                confidence=0.98,
            )
            make_screenshot_if_error(check.is_not_none(screenshot))
