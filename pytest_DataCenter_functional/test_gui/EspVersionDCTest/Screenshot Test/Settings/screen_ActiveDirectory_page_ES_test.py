...
def handle_sql_connection_settings(start_screen_test_page_es):
...
@pytest.fixture(scope="class")
def handle_add_domain(start_screen_test_page_es):
    send_keys("{VK_MENU down}Y3Y5{VK_MENU up}")
    app = start_screen_test_page_es
    handle = app.window(name_re="DataCenter*")
    handle.by(
...
@pytest.fixture(scope="class")
def handle_add_workgroup_user(start_screen_test_page_es):
    send_keys("{VK_MENU down}Y3Y5{VK_MENU up}")
    app = start_screen_test_page_es
    handle = app.window(name_re="DataCenter*")
    handle.by(
...
@pytest.fixture(scope="class")
def handle_add_internal_user(start_screen_test_page_es):
    send_keys("{VK_MENU down}Y3Y5{VK_MENU up}")
    app = start_screen_test_page_es
    handle = app.window(name_re="DataCenter*")
    handle.by(
...
class TestActiveDirectoryPage:
    ...
    class TestActiveDirectoryMainPage:
        def test_active_directory_screen(self, start_screen_test_page_es):
            send_keys("{VK_MENU down}Y3Y5{VK_MENU up}")
            time.sleep(2)
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.active_directory["path"],
    ...
    class TestSQLConnectionSettingsWindow:
        def test_sql_connection_settings_screen(self, handle_sql_connection_settings):
            send_keys("{VK_MENU down}Y3Y5{VK_MENU up}")
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.active_directory_active_directory_sql_connection_settings[
                    "path"
                ],
                confidence=LevelOfConfidence.confidence["reduced_confidence"],
            )
    ...
    class TestAddDomainWindow:
        def test_add_domain_screen(self, handle_add_domain):
            send_keys("{VK_MENU down}Y3Y5{VK_MENU up}")
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.active_directory_active_directory_add_domain[
                    "path"
                ],
    ...
    class TestAddWorkgroupUserWindow:
        def test_add_workgroup_user_screen(self, handle_add_workgroup_user):
            send_keys("{VK_MENU down}Y3Y5{VK_MENU up}")
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.active_directory_active_directory_add_workgroup_user[
                    "path"
                ],
                confidence=LevelOfConfidence.confidence["confidence"],
            )
    ...
    class TestAddInternalUserWindow:
        def test_add_internal_user_screen(self, handle_add_internal_user):
            send_keys("{VK_MENU down}Y3Y5{VK_MENU up}")
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsES.active_directory_active_directory_add_internal_user[
                    "path"
                ],
                confidence=LevelOfConfidence.confidence["confidence"],
            )
