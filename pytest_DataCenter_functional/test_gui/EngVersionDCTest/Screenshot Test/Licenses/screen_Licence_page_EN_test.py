...
def handle_licence_key(start_screen_test_page_en):
...
@pytest.fixture(scope="class")
def handle_auto_license(start_screen_test_page_en):
...
@pytest.fixture(scope="class")
def handle_statistic_license(start_screen_test_page_en):
...
@pytest.mark.testGUI_EngVersionDCTest_ScreensTest_Licenses
class TestLicensePage:
    ...
    class TestLicenseMainPage:
        def test_license_page_screen(self, start_screen_test_page_en):
            ...
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsEN.licenses_page["path"],
    ...
    class TestLicenseUpdateLicenseWindow:
        def test_licence_key(self, handle_licence_key):
            time.sleep(2)
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsEN.license_page_update_license_window["path"],
                confidence=LevelOfConfidence.confidence["reduced_confidence"],
            )
            make_screenshot_if_error(check.is_not_none(screenshot))
    ...
    class TestLicenseDistributeAutomaticallyWindow:
        def test_auto_license(self, handle_auto_license):
            time.sleep(2)
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsEN.license_page_distribute_automatically_window[
                    "path"
                ],
    ...
    class TestLicenseStatisticsWindow:
        def test_handle_statistic_license(self, handle_statistic_license):
            time.sleep(6)
            screenshot = pyautogui.locateOnScreen(
                PathToScreenshotsEN.license_page_statistics_window["path"],
                confidence=LevelOfConfidence.confidence["reduced_confidence"],
            )
            make_screenshot_if_error(check.is_not_none(screenshot))
