...
@pytest.fixture(scope="class")
def handle_add_server(start_test_page_en):
    send_keys("{VK_MENU down}Y3Y2{VK_MENU up}")
    app = start_test_page_en
    handle = app.window(name_re="DataCenter*")
    handle.by(name=DefaultDatabasePage.add_button["title_en"]).click_input()
    yield handle
    send_keys("%{F4}")


@pytest.fixture(scope="class")
def handle_edit_server(start_test_page_en):
    send_keys("{VK_MENU down}Y3Y2{VK_MENU up}")
    app = start_test_page_en
    handle = app.window(name_re="DataCenter*")
    handle.by(name=DefaultDatabasePage.edit_button["title_en"]).click_input()
    yield handle
    send_keys("%{F4}")


@pytest.fixture(scope="class")
def handle_delete_server(start_test_page_en):
    send_keys("{VK_MENU down}Y3Y2{VK_MENU up}")
    app = start_test_page_en
    handle = app.window(name_re="DataCenter*")
    handle.by(name=DefaultDatabasePage.delete_button["title_en"]).click_input()
    yield handle
    send_keys("%{F4}")
...
class TestDefaultDatabasePage:
    ...
    class TestDefaultDatabaseMainPage:
        def test_cancel_button(self, handle):
            cancel_button = (
                handle.by(name=DefaultDatabasePage.cancel_button["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_apply_button(self, handle):
            apply_button = (
                handle.by(name=DefaultDatabasePage.apply_button["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_edit_button(self, handle):
            edit_button = (
                handle.by(name=DefaultDatabasePage.edit_button["title_en"])
                .find()
                .window_text()
            )
            check.equal(edit_button, "Edit")

        def test_delete_button(self, handle):
            delete_button = (
                handle.by(name=DefaultDatabasePage.delete_button["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_add_button(self, handle):
            add_button = (
                handle.by(name=DefaultDatabasePage.add_button["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_connection_settings(self, handle):
            connection_settings = (
                handle.by(name=DefaultDatabasePage.connection_settings["title_en"])
                .find()
                .window_text()
            )
            check.equal(
                connection_settings,
                "Make the list of servers, specify their connection settings and parameters of creating database name.",
            )

        def test_servers(self, handle):
            servers = (
                handle.by(name=DefaultDatabasePage.servers["title_en"])
                .find()
                .window_text()
            )
            check.equal(servers, "Servers")

        def test_create_database(self, handle):
            create_database = (
                handle.by(name=DefaultDatabasePage.create_database["title_en"])
                .find()
                .window_text()
            )
            check.equal(
                create_database,
                "Create database name based on the following parameters",
            )

        def test_smtp_servers_integration(self, handle):
            smtp_servers_integration = (
                handle.by(name=DefaultDatabasePage.smtp_servers_integration["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_datacenter(self, handle):
            datacenter = (
                handle.by(name=DefaultDatabasePage.datacenter["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_mail_servers_integration(self, handle):
            mail_servers_integration = (
                handle.by(name=DefaultDatabasePage.mail_servers_integration["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_reportcenter(self, handle):
            reportcenter = (
                handle.by(name=DefaultDatabasePage.reportcenter["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_alertcenter(self, handle):
            alertcenter = (
                handle.by(name=DefaultDatabasePage.alertcenter["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_endpointcontroller(self, handle):
            endpointcontroller = (
                handle.by(name=DefaultDatabasePage.endpointcontroller["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_networkcontroller(self, handle):
            networkcontroller = (
                handle.by(name=DefaultDatabasePage.networkcontroller["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_component_parameter(self, handle):
            component_parameter = (
                handle.by(name=DefaultDatabasePage.component_parameter["title_en"])
                .find()
                .window_text()
            )
            check.equal(component_parameter, "Component parameter (product, protocol)")

        def test_db_suffix_center_components(self, handle):
            db_suffix_center_components = (
                handle.by(
                    name=DefaultDatabasePage.db_suffix_center_components["title_en"]
                )
                .find()
                .window_text()
            )
            check.equal(db_suffix_center_components, "DB suffix for Center components")

        def test_db_suffix_controller_components(self, handle):
            db_suffix_controller_components = (
                handle.by(
                    name=DefaultDatabasePage.db_suffix_controller_components["title_en"]
                )
                .find()
                .window_text()
            )
            check.equal(
                db_suffix_controller_components,
                "DB suffix for Controller components      ",
            )

        def test_component_id(self, handle):
            component_id = (
                handle.by(name=DefaultDatabasePage.component_id["title_en"])
                .find()
                .window_text()
            )
            check.equal(component_id, "Component ID")

        def test_prefix(self, handle):
            prefix = (
                handle.by(name=DefaultDatabasePage.prefix["title_en"])
                .find()
                .window_text()
            )
            check.equal(prefix, "Prefix")

        def test_server_with_installed_component(self, handle):
            server_with_installed_component = (
                handle.by(
                    name=DefaultDatabasePage.server_with_installed_component["title_en"]
                )
                .find()
                .window_text()
            )
            check.equal(
                server_with_installed_component, "Server with the installed component"
            )
    ...
    class TestDefaultDatabaseAddServerWindow:
        ...
        def test_add_server_window(self, handle_add_server):
            add_server = (
                handle_add_server.by(name=DefaultDatabasePage.add_server["title_en"])
                .find()
                .window_text()
            )
            check.equal(add_server, "Add server")

        def test_connection_settings2(self, handle_add_server):
            connection_settings2 = (
                handle_add_server.by(
                    name=DefaultDatabasePage.connection_settings2["title_en"]
                )
                .find()
                .window_text()
            )
        ...
        def test_check_connection(self, handle_add_server):
            check_connection = (
                handle_add_server.by(
                    name=DefaultDatabasePage.check_connection["title_en"]
                )
                .find()
                .window_text()
            )
        ...
        def test_sql_server_authentication_mode(self, handle_add_server):
            sql_server_authentication_mode = (
                handle_add_server.by(
                    name=DefaultDatabasePage.sql_server_authentication_mode["title_en"]
                )
                .find()
                .window_text()
            )
        ...
        def test_windows_authentication_mode(self, handle_add_server):
            windows_authentication_mode = (
                handle_add_server.by(
                    name=DefaultDatabasePage.windows_authentication_mode["title_en"]
                )
                .find()
                .window_text()
            )
        ...
        def test_password(self, handle_add_server):
            password = (
                handle_add_server.by(name=DefaultDatabasePage.password["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_username(self, handle_add_server):
            username = (
                handle_add_server.by(name=DefaultDatabasePage.username["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_cancel(self, handle_add_server):
            cancel = (
                handle_add_server.by(
                    name=DefaultDatabasePage.cancel["title_en"],
        ...
        def test_add(self, handle_add_server):
            add = (
                handle_add_server.by(
                    name=DefaultDatabasePage.add["title_en"],
                    found_index=DefaultDatabasePage.cancel["found_index"],
                )
                .find()
                .window_text()
        ...
        def test_server_name(self, handle_add_server):
            server_name = (
                handle_add_server.by(name=DefaultDatabasePage.server_name["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_server_type(self, handle_add_server):
            server_type = (
                handle_add_server.by(name=DefaultDatabasePage.server_type["title_en"])
                .find()
                .window_text()
            )
    ...
    class TestDefaultDatabaseEditServerWindow:
        ...
        def test_connection_settings3(self, handle_edit_server):
            connection_settings3 = (
                handle_edit_server.by(
                    name=DefaultDatabasePage.connection_settings3["title_en"]
                )
                .find()
                .window_text()
            )
            check.equal(connection_settings3, "Connection settings")

        def test_check_connection2(self, handle_edit_server):
            check_connection2 = (
                handle_edit_server.by(
                    name=DefaultDatabasePage.check_connection2["title_en"]
                )
                .find()
                .window_text()
            )
        ...
        def test_sql_server_authentication_mode2(self, handle_edit_server):
            sql_server_authentication_mode2 = (
                handle_edit_server.by(
                    name=DefaultDatabasePage.sql_server_authentication_mode2["title_en"]
                )
                .find()
                .window_text()
            )
            check.equal(
                sql_server_authentication_mode2, "SQL Server Authentication mode"
            )

        def test_windows_authentication_mode2(self, handle_edit_server):
            windows_authentication_mode2 = (
                handle_edit_server.by(
                    name=DefaultDatabasePage.windows_authentication_mode2["title_en"]
                )
                .find()
                .window_text()
            )
            check.equal(windows_authentication_mode2, "Windows Authentication mode")

        def test_password2(self, handle_edit_server):
            password2 = (
                handle_edit_server.by(name=DefaultDatabasePage.password2["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_username2(self, handle_edit_server):
            username2 = (
                handle_edit_server.by(name=DefaultDatabasePage.username2["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_cancel2(self, handle_edit_server):
            cancel2 = (
                handle_edit_server.by(
                    name=DefaultDatabasePage.cancel2["title_en"],
                    found_index=DefaultDatabasePage.cancel["found_index"],
                )
                .find()
                .window_text()
        ...
        def test_ok_button(self, handle_edit_server):
            ok_button = (
                handle_edit_server.by(
                    name=DefaultDatabasePage.ok_button["title_en"],
                    found_index=DefaultDatabasePage.cancel["found_index"],
                )
                .find()
                .window_text()
        ...
        def test_server_name2(self, handle_edit_server):
            server_name2 = (
                handle_edit_server.by(name=DefaultDatabasePage.server_name2["title_en"])
                .find()
                .window_text()
            )
            check.equal(server_name2, "Server name:")

        def test_server_type2(self, handle_edit_server):
            server_type2 = (
                handle_edit_server.by(name=DefaultDatabasePage.server_type2["title_en"])
                .find()
                .window_text()
            )
            check.equal(server_type2, "Server type:")
    ...
    class TestDefaultDatabaseDeleteServerWindow:
        ...
        def test_confirm(self, handle_delete_server):
            confirm = (
                handle_delete_server.by(name=DefaultDatabasePage.confirm["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_no_button(self, handle_delete_server):
            no_button = (
                handle_delete_server.by(name=DefaultDatabasePage.no_button["title_en"])
                .find()
                .window_text()
            )
        ...
        def test_yes_button(self, handle_delete_server):
            yes_button = (
                handle_delete_server.by(name=DefaultDatabasePage.yes_button["title_en"])
                .find()
                .window_text()
            )
